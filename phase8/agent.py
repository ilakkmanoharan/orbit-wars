"""Phase 8 — Phase 2 sim picker + comet paths + static/comet scoring."""
import math
...
def _target_value(tgt, comet_pids, step, eta):
    val = tgt.production / (eta + 1)
    if is_static_planet(tgt.x, tgt.y, tgt.radius):
        val *= 1.25
...
def _build_moves(planets, player, init, av, comet_pids, comet_groups, step, policy):
    ...
    for src in my:
        ...
        for tgt in targets:
            ...
            if policy == "conservative" and (tgt.owner != -1 or eta > 20):
                continue
            val = _target_value(tgt, comet_pids, step, eta)
    ...
    for val, src, tgt, needed, eta in scored:
        ...
        angle, _ = launch_angle(
            src.x, src.y, tgt.x, tgt.y, tgt.id, init, av, tgt.radius, needed, comet_groups,
        )
...
def decide(obs):
    player, planets, av, init, comet_pids, comet_groups, step = _parse_obs(obs)
    ...
    for policy in POLICIES:
        moves = _build_moves(planets, player, init, av, comet_pids, comet_groups, step, policy)
