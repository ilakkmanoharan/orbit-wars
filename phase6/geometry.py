"""Geometry helpers (Phase 6) — phase2 + comet paths."""
import math
...
def predict_position(planet_id, initial_planets, angular_velocity, steps_ahead,
    ...
    if comet_groups and comet_group_for(planet_id, comet_groups):
...
def optimize_fleet_size(sx, sy, tx, ty, tr, min_ships, max_ships, pid, init_map, av,
                        radius, comet_groups=None):
    """Pick fleet size minimizing ETA for high-value distant targets."""
    best_ships = min_ships
    best_eta = estimate_eta(sx, sy, tx, ty, tr, min_ships, pid, init_map, av, radius, comet_groups)
    if max_ships <= min_ships:
        return best_ships, best_eta
    step = max(1, (max_ships - min_ships) // 4)
    for ships in range(min_ships + step, max_ships + 1, step):
        eta = estimate_eta(sx, sy, tx, ty, tr, ships, pid, init_map, av, radius, comet_groups)
        if eta < best_eta:
            best_eta = eta
            best_ships = ships
    return best_ships, best_eta
...
def in_comet_window(step):
    return any(0 <= spawn - step <= COMET_WINDOW for spawn in COMET_SPAWN_STEPS)
