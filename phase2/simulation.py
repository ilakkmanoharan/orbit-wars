"""Simplified forward simulator for policy evaluation."""
import copy
import math

from geometry import CENTER, ROTATION_RADIUS_LIMIT, fleet_speed, predict_position
...
def state_from_obs(obs, player):
    ...
    planets = [
        SimPlanet(p[0], p[1], p[2], p[3], p[4], p[5], p[6]) for p in raw
    ]
    return planets, [], av, init, player
...
def _opponent_moves(planets, player, init_map, av):
    """Crude model: non-player planets send half garrison to nearest neutral."""
    moves = []
...
def simulate(planets, fleets, player, init_map, av, our_moves, depth=12):
    ...
    for _ in range(depth):
        ...
        apply_launches(planets, fleets, our_moves if step == 1 else [], player)
