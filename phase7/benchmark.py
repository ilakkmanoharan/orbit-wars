#!/usr/bin/env python3
"""Benchmark phase7 vs phase2 (requires orbit_wars env)."""
import argparse
import os
import sys
import time
...
PHASE7 = os.path.join(ROOT, "main.py")
...
def main():
    ...
    parser.add_argument("--vs", default="phase2")
    ...
    opponent = PHASE2 if args.vs == "phase2" else PHASE7
    agent_a = PHASE7 if args.vs == "phase2" else PHASE2
    seeds = list(range(args.seeds))
    wins, ties, total = win_rate(agent_a, opponent, seeds)
    label = "Phase 7 vs Phase 2" if args.vs == "phase2" else "Phase 2 vs Phase 7"
    print(f"{label}: {wins}/{total} wins, {ties} ties ({wins/total:.0%})")
