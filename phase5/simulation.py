"""Forward simulator with horizon-weighted evaluation (Phase 5)."""
import copy
import math

from geometry import MAX_STEPS, fleet_speed, predict_position
...
def state_from_obs(obs, player):
    ...
    return planets, fleets, av, init, player, step


def evaluate(planets, fleets, player, step=0):
    ...
    prod_value = sum(p.production * remaining for p in planets if p.owner == player)
    return prod_value * 0.15 + prod * 15 + ships + owned * 8
...
def _opponent_moves(planets, player, init_map, av):
    ...
    for p in planets:
        ...
        if p.ships < 4:
            continue
        # Prefer neutrals, but sometimes push weak enemy planets
        if neutrals:
            nearest = min(neutrals, key=lambda n: math.hypot(p.x - n.x, p.y - n.y))
        elif my_planets:
            nearest = min(my_planets, key=lambda n: math.hypot(p.x - n.x, p.y - n.y))
            if nearest.ships > p.ships * 2:
                continue
        else:
            continue
        send = max(2, p.ships // 2)
...
def simulate(planets, fleets, player, init_map, av, our_moves, depth=18, step=0):
    ...
    for _ in range(depth):
        ...
        fleets = _move_fleets(planets, fleets, init_map, av, cur_step)
    return evaluate(planets, fleets, player, cur_step)
